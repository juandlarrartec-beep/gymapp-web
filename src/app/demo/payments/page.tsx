...
export default async function DemoPaymentsPage() {
  ...
  return (
    <div className="p-8">
                ...
                <div className="text-right">
                  {debtor.payments[0] && (
                    <p className="text-sm font-medium text-red-600">
                      {(debtor.payments[0].amount / 100).toLocaleString("es-AR")}{" "}
                      {debtor.payments[0].currency}
                    </p>
                  )}
      ...
      {/* Tabla de cobros — read-only, sin acciones */}
          ...
          <tbody className="divide-y">
            {payments.map((payment) => (
              <tr key={payment.id} className="hover:bg-slate-50">
                ...
                <td className="px-4 py-3">
                  <span
                    className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${statusClass[payment.status]}`}
                  >
