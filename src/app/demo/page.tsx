...
export default async function DemoPage() {
  ...
  if (!gym) throw new Error("Gym demo no encontrado")
  ...
  const [
    ...
    db.member.findMany({
      where: {
        ...
        accessLogs: { none: { timestamp: { gte: tenDaysAgo } } },
      },
      select: { id: true },
      take: 100,
    }),
  ...
  const retentionLastMonth = activeMembersLastMonth > 0 ? 100 : 100
  ...
  const memberFlow: MemberFlowDataPoint[] = monthRanges.slice(-6).map((range, i) => {
    const altas = i === 0 ? newLastMonth : 0
  ...
  if (overdueCount > 0) {
    alerts.push({
      ...
      action: { label: "Ver pagos vencidos", href: "/demo/payments" },
    })
  }
  ...
  if (churnHighCount > 0) {
    alerts.push({
      ...
      action: { label: "Ver riesgo de fuga", href: "/demo" },
    })
  }
  ...
  if (inactiveMembersRaw.length > 0) {
    alerts.push({
      ...
      action: { label: "Ver socios", href: "/demo/members" },
    })
  }
  ...
  if (lowOccupancyClasses.length > 0) {
    alerts.push({
      ...
      action: { label: "Ver clases", href: "/demo/classes" },
    })
  }
  ...
  const lastAccessMemberName =
    lastLog?.member ? `${lastLog.member.firstName} ${lastLog.member.lastName}` : null
  ...
  return (
    <div className="p-6 space-y-6 bg-slate-50 dark:bg-slate-950 min-h-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* QuickActions omitido en demo — apuntarían a rutas del dashboard real */}
